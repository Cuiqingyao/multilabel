...
class HISO(nn.Module):
    def __init__(self, opt):
        ...
        # conv layer
        self.fconv1d = [nn.Conv1d(in_channels=opt.embed_dim, out_channels=96, kernel_size=3, padding=1).cuda(),
                        nn.Conv1d(in_channels=opt.embed_dim, out_channels=96, kernel_size=4, padding=1).cuda(),
                        nn.Conv1d(in_channels=opt.embed_dim, out_channels=96, kernel_size=5, padding=2).cuda()]
        ...
        # Bi-GRU Layer
        self.wd_bi_gru = nn.GRU(input_size = 288,
        ...
        # Bi-GRU Layer
        self.pos_bi_gru = nn.GRU(input_size = 288,
                hidden_size = opt.ghid_size,
                num_layers = opt.glayer,
                bias = True, 
                batch_first = True,
